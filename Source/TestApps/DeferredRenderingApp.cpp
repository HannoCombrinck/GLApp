...
namespace baselib {
	...
	namespace
	{
		std::shared_ptr<Shader> createSimpleShader(const std::string& sPipelineName, const std::string& sVertexShader, const std::string& sFragmentShader)
		{
	...
	void DeferredRenderingApp::onInit(int iWidth, int iHeight)
	{
		...
		std::vector<std::shared_ptr<Texture>> aTargets;
		...
		std::shared_ptr<Visual> spSphereVisual = null_ptr;
		spUnitSphere->traverse([&spSphereVisual] (const std::shared_ptr<Spatial>& sp) {
			if (auto spVisual = std::dynamic_pointer_cast<Visual>(sp))
